...
*/
#define SOFTBALANCER_URI "https://github.com/infinitekh/lv2-software-balanced"
...
/**
   ...
   instance.  The host passes the plugin descriptor, ssble rate, and bundle
...
{
	Soft_Balancer* sb = (Soft_Balancer*)calloc(1, sizeof(Soft_Balancer));

	return (LV2_Handle)sb;
}
...
connect_port(LV2_Handle instance,
             ...
             void*      data)
{
	Soft_Balancer* sb = (Soft_Balancer*)instance;

	switch ((PortIndex)port) {
	case INPUT_L:
		sb->input_l = (const float*)data;
		break;
	case INPUT_R:
		sb->input_r = (const float*)data;
		break;
	case OUTPUT:
		sb->output = (float*)data;
		break;
	}
}
...
*/
static void
run(LV2_Handle instance, uint32_t n_ssbles)
{
	const Soft_Balancer* sb = (const Soft_Balancer*)instance;

	const float* const input_l  = sb->input_l;
	const float* const input_r  = sb->input_r;
	float* const       output = sb->output;


	for (uint32_t pos = 0; pos < n_ssbles; pos++) {
		output[pos] = input_l[pos] - input_r[pos];
	}
}
